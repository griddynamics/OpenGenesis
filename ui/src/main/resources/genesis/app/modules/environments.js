...
function (genesis, backend, poller, status, EnvStatus, Backbone, $) {
  ...
  Environments.Collection = Backbone.Collection.extend({
    ...
    parse: function(json) {
      if (json.items) {
        this._createLink = _(json.links).find(function(link) { return link.type == backend.LinkTypes.Environment && _(link.methods).contains("post") } );
        return json.items
      } else {
        return json;
      }
...
//        if(view.project.canAccessProperties()) {
