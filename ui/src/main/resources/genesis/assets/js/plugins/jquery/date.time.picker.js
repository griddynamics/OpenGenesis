 ...
 */
/*jslint evil: true, white: false, undef: false, nomen: false */ (function($) {
  ...
  $.extend(Timepicker.prototype, {
    ...
    _injectTimePicker: function() {
      ...
      if ($dp.find("div.ui-timepicker-div")
        .length === 0 && o.showTimepicker) {
        ...
        for (var i = 0, l = this.units.length; i < l; i++) {
          litem = this.units[i];
          uitem = litem.substr(0, 1)
            .toUpperCase() + litem.substr(1);
        ...
        if (this._defaults.addSliderAccess) {
          ...
          setTimeout(function() { // fix for inline mode
            if ($tp.find('.ui-slider-access')
              .length === 0) {
              $tp.find('.ui-slider:visible')
                .sliderAccess(sliderAccessArgs);

              // fix any grids since sliders are shorter
              var sliderAccessWidth = $tp.find('.ui-slider-access:eq(0)')
                .outerWidth(true);
              if (sliderAccessWidth) {
                $tp.find('table:visible')
                  .each(function() {
                    var $g = $(this),
                      oldWidth = $g.outerWidth(),
                      oldMarginLeft = $g.css(rtl ? 'marginRight' : 'marginLeft')
                        .toString()
                        .replace('%', ''),
  ...
  $.fn.extend({
    ...
    datetimepicker: function(o) {
      ...
      if (typeof(o) == 'string') {
        ...
        return this.each(function() {
          var $t = $(this);
          $t.datepicker($.timepicker._newInst($t, o)
            ._defaults);
        });
      }
    }
  ...
  $.datepicker.parseTime = function(timeFormat, timeString, options) {
    ...
    var strictParse = function(f, s, o) {
      ...
      var getFormatPositions = function(timeFormat) {
        var finds = timeFormat.toLowerCase()
        ...
        if (finds) {
          for (var i = 0; i < finds.length; i++) {
            if (orders[finds[i].toString()
              .charAt(0)] == -1) {
              orders[finds[i].toString()
                .charAt(0)] = i + 1;
            }
          }
        }
    ...
    tmptime = tmptime.replace(/(?:HH?|hh?|mm?|ss?|[tT]{1,2}|[lz]|('.*?'|".*?"))/g, function(match) {
      switch (match) {
        ...
        case 's':
          return time.second;
        case 'l':
          return ('00' + time.millisec)
            .slice(-3);
        case 'z':
          return time.timezone === null ? options.defaultTimezone : time.timezone;
        case 'T':
          return ampmName.charAt(0)
            .toUpperCase();
        case 'TT':
  ...
  $.datepicker._getDateDatepicker = function(target, noDefault) {
    ...
    if (tp_inst) {
      ...
      if (date && tp_inst._parseTime($(target)
        .val(), tp_inst.timeOnly)) {
  ...
  var splitDateTime = function(dateFormat, dateTimeString, dateSettings, timeSettings) {
    try {
      ...
      if (allPartsLen > 1) {
        return [
          allParts.splice(0, allPartsLen - timePartsLen)
            .join(separator),
          allParts.splice(0, timePartsLen)
            .join(separator)];
      }
  ...
  $.timepicker.timeZoneOffsetString = function(date) {
    ...
    return (off >= 0 ? '+' : '-') + ('0' + (hours * 101)
      .toString())
      .slice(-2) + ('0' + (minutes * 101)
      .toString())
      .slice(-2);
  };
      ...
      $([startTime, endTime])
        .each(function() {
          ...
          if ($(this)
            .val() && date) {
            $(this)
              .val($.datepicker.formatDate(format, date));
          }
        });
    }
