define ["genesis", "backbone", "modules/settings/users", "modules/status", "modules/validation", "services/backend", "jquery", "jvalidate"], (genesis, Backbone, Users, status, validation, backend, $) ->
  ...
  class Groups.Views.Main extends Backbone.View
    ...
    initialize: ->
      _.bind @render, this
      @collection = new Users.Collections.Groups()
  ...
  class GroupsEdit extends Backbone.View
    ...
    initialize: (options) ->
      ...
      @users = new Users.Collections.Users()
      @groupRolesArray = []
      unless @group.isNew()
