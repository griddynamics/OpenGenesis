...
import com.griddynamics.genesis.rest.links.{LinkBuilder, WebPath}
...
class WorkflowController extends RestApiExceptionsHandler {
  ...
  def workflowStats(request: HttpServletRequest) = genesisRestService.workflowStats.map { stat =>
    val top = WebPath(absolutePath(s"/rest/projects/${stat.projectId}")(request))
    stat.withLinks(
      LinkBuilder(top / "workflows/running", LinkTarget.SELF, classOf[WorkflowDetails], GET)
    ).filtered()
  }
  ...
  def runningWorkflows(@PathVariable(value = "projectId") projectId: Int, request: HttpServletRequest) : List[WorkflowDetails] = {
    ...
    List(WorkflowDetails(mockProject, "mock", "Running", "nodoby", Map("foo" -> "bar"), None, None, Some(0), None))
  }
