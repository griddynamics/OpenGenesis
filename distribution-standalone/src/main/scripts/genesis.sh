...
# Set this to the root of the Genesis installation
GENESIS_HOME=".."

# If specified, the Wrapper will be run as the specified user.

# IMPORTANT - Make sure that the user has the required privileges to write into the Genesis installation directory.
...
# Test if GENESIS_HOME is relative (does not start with /). If relative it should be relative to this script location
FIRST_CHAR=`echo $GENESIS_HOME | cut -c1,1`
if [ "$FIRST_CHAR" != "/" ]
then
    GENESIS_HOME=$REALDIR/$GENESIS_HOME
fi
...
# If the PIDDIR is relative, set its value relative to the full GENESIS_HOME to avoid problems if
#  the working directory is later changed.
FIRST_CHAR=`echo $PIDDIR | cut -c1,1`
if [ "$FIRST_CHAR" != "/" ]
then
    PIDDIR=$GENESIS_HOME/$PIDDIR
fi
...
WRAPPER_CMD="$GENESIS_HOME/bin/jsw/$DIST_OS-$DIST_ARCH-$DIST_BITS/wrapper"
if [ ! -x "$WRAPPER_CMD" ]; then
    echo "Missing platform binary: $WRAPPER_CMD"
    exit 1
fi

WRAPPER_CONF="$GENESIS_HOME/bin/jsw/conf/wrapper.conf"
